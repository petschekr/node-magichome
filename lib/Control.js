...
class Control {
	...
	_handleCommandWithSuccess(cmd_buf) {
	...
	turnOn() {
		...
		return this._handleCommandWithSuccess(cmd_buf);
	}
	 ...
	 */
	setColorWithBrightness(red, green, blue, brightness) {
	...
	setPattern(pattern, speed) {
		...
		if (pattern_code == undefined) return Promise.reject(new Error("Invalid pattern"));
	...
	queryState() {
		return new Promise((resolve, reject) => {
			...
			this._sendCommand(cmd_buf, true, function (err, data) {
				if (err) return reject(err);
				if (data.length < 14) return reject(new Error("Only got short reply"));
